...
// const checkPassword = function (minVal, maxVal, checkChar, word) {
//   console.log(minVal,maxVal, checkChar, word)
//   if (word.includes(checkChar)) {
//     let count = countLetterInWord(word, checkChar);
...
const checkPassword = function (allowIndex, allowIndex2, checkChar, word) {
  if (word.includes(checkChar)) {
    if (word[allowIndex - 1] === checkChar) {
      if (word[allowIndex2 - 1] !== checkChar) {
        return true;
      }
      return false;
    }
    else if (word[allowIndex2 - 1] === checkChar) {
      if (word[allowIndex - 1] !== checkChar) {
        return true;
      }
      return false;
...
function countLetterInWord(word, checkChar) {
  let sum = 0;
...
input.filter((data) => {
  ...
  let min = data.split(' ')[0].split('-')[0];
  // let min = data[0];
  let max = data.split(' ')[0].split('-')[1];

  // let max = data[2];
  let mainChar = data.split(' ')[1].split(':')[0];
  let string = data.split(' ')[2];
  // checkPassword(min, max, mainChar, string);

  if (checkPassword(min, max, mainChar, string)) {
    totalPassed = totalPassed + 1;
  }
